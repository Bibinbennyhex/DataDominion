...
def process_case_iii_using_latest_history_context(
    ...
    if latest_count > 0:
        ...
        write_case_table_bucketed(
            spark=spark,
            df=latest_patch_df,
            table_name="temp_catalog.checkpointdb.case_3_latest_month_patch",
            config=config,
...
def process_case_iii(spark: SparkSession, case_iii_df, config: Dict[str, Any], expected_rows: Optional[int] = None):
    ...
    if split_enabled and not bool(config.get("_case3_split_internal", False)):
        ...
        if global_latest_month is not None:
            ...
            split_case_tables = {
                ...
                "temp_catalog.checkpointdb.case_3_latest_month_patch": "case_3_latest_month_patch_temp_split_combine",
    ...
    if latest_count > 0:
        ...
        logger.info("Case III Part 2 generated: temp_catalog.checkpointdb.case_3_latest_month_patch")
        logger.info("-" * 60)
    else:
...
def process_case_iii_soft_delete(
    ...
    if all(c in latest_cols for c in history_cols):
        ...
        if not latest_base.isEmpty():
            ...
            logger.info("Case III Soft Delete - Latest history patches generated")
...
def write_backfill_results(spark: SparkSession, config: Dict[str, Any], expected_rows_append: Optional[int] = None):
    ...
    try:
        ...
        if spark.catalog.tableExists("temp_catalog.checkpointdb.case_3_latest_month_patch"):
            case_3_latest_month_patch_df = spark.read.table("temp_catalog.checkpointdb.case_3_latest_month_patch")
        ...
        process_start_time = time.time()
        if spark.catalog.tableExists("temp_catalog.checkpointdb.case_3d_latest_history_context_patch"):
            latest_case_3d_history_df = spark.read.table("temp_catalog.checkpointdb.case_3d_latest_history_context_patch")
