...
def apply_backfill_updates(spark: SparkSession, backfill_updates_df, config: Dict[str, Any]):
    ...
    # Update base_ts if provided (propagate max timestamp from backfills)
    if 'new_base_ts' in backfill_updates_df.columns:
        ts_col = config.get('max_identifier_column', 'base_ts')
        update_columns.append(f"t.{ts_col} = GREATEST(t.{ts_col}, s.new_base_ts)")
