...
def setup_tables(spark):
    ...
    spark.sql("CREATE DATABASE IF NOT EXISTS demo.test_recovery")
    
    # Create temp schema for temp tables (v9.4)
    spark.sql("CREATE DATABASE IF NOT EXISTS demo.temp")
    
    # Drop existing tables
    spark.sql("DROP TABLE IF EXISTS demo.test_recovery.accounts")
    spark.sql("DROP TABLE IF EXISTS demo.test_recovery.summary")
    spark.sql("DROP TABLE IF EXISTS demo.test_recovery.latest_summary")
    
    # Create accounts table (source)
    spark.sql("""
        CREATE TABLE demo.test_recovery.accounts (
    ...
    spark.sql("""
        CREATE TABLE demo.test_recovery.summary (
            cons_acct_key BIGINT,
            rpt_as_of_mo STRING,
            base_ts TIMESTAMP,
            balance_am INT,
    ...
    spark.sql("""
        CREATE TABLE demo.test_recovery.latest_summary (
            cons_acct_key BIGINT,
            rpt_as_of_mo STRING,
            base_ts TIMESTAMP,
            balance_am INT,
...
def setup_existing_data(spark):
    ...
    spark.sql(f"""
        INSERT INTO demo.test_recovery.summary VALUES
    ...
    spark.sql(f"""
        INSERT INTO demo.test_recovery.latest_summary VALUES
        (2001, '2026-02', TIMESTAMP '2026-02-15 10:00:00',
         5000, 500, 10000, 0, 0, 'A', '0',
         {build_int_array({0: 500, 1: 450})},
         {build_int_array({0: 5000, 1: 4500})},
    ...
    print(f"  Summary rows: {spark.sql('SELECT COUNT(*) FROM demo.test_recovery.summary').first()[0]}")
    print(f"  Latest summary rows: {spark.sql('SELECT COUNT(*) FROM demo.test_recovery.latest_summary').first()[0]}")
...
def insert_test_accounts(spark):
    ...
    spark.sql(f"""
        INSERT INTO demo.test_recovery.accounts VALUES
    ...
    spark.sql("""
        ...
        FROM demo.test_recovery.accounts
...
def run_pipeline(spark):
    ...
    config = {
        ...
        "source_table": "demo.test_recovery.accounts",
        "destination_table": "demo.test_recovery.summary",
        "latest_history_table": "demo.test_recovery.latest_summary",
...
def verify_case_i(spark):
    ...
    result = spark.sql("""
        SELECT * FROM demo.test_recovery.summary
...
def verify_case_ii(spark):
    ...
    if result.count() == 1:
        ...
        latest = spark.sql("""
            SELECT rpt_as_of_mo FROM demo.test_recovery.latest_summary
...
def verify_case_iii(spark):
    ...
    total = spark.sql("""
        SELECT COUNT(*) as cnt FROM demo.test_recovery.summary
...
def verify_case_iv(spark):
    ...
    jan = spark.sql("""
        SELECT balance_am_history FROM demo.test_recovery.summary
...
def verify_gaps(spark):
    ...
    spark.sql("""
        ...
        FROM demo.test_recovery.summary
        WHERE cons_acct_key = 5001
