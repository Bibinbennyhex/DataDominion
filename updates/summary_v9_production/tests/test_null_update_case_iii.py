...
def setup_data(spark):
    print("Setting up Null Update Scenario (Case III)...")
    
    spark.sql("CREATE DATABASE IF NOT EXISTS demo.null_test_iii")
    
    # 1. Create Tables
    spark.sql("DROP TABLE IF EXISTS demo.null_test_iii.accounts")
    spark.sql("""
        CREATE TABLE demo.null_test_iii.accounts (
    ...
    """
    spark.sql("DROP TABLE IF EXISTS demo.null_test_iii.summary")
    spark.sql(f"CREATE TABLE demo.null_test_iii.summary ({summary_schema}) USING iceberg PARTITIONED BY (rpt_as_of_mo)")
    
    spark.sql("DROP TABLE IF EXISTS demo.null_test_iii.latest_summary")
    spark.sql(f"CREATE TABLE demo.null_test_iii.latest_summary ({summary_schema}) USING iceberg")

    # 2. Insert Existing Records
    print("Inserting existing records...")
    spark.sql("""
        INSERT INTO demo.null_test_iii.summary VALUES
    ...
    """)
    spark.sql("""
        INSERT INTO demo.null_test_iii.latest_summary VALUES
        ('9999', DATE '2025-11-30', 24311, timestamp '2025-11-01 00:00:00', 0.0, array(0.0, null, 49.0), timestamp '2025-11-01 00:00:00')
    """)
    ...
    spark.sql("""
        INSERT INTO demo.null_test_iii.accounts VALUES
...
def verify_results(spark):
    print("\n==================================================")
    print("VERIFYING NULL UPDATES (CASE III)")
    print("==================================================")
    
    df = spark.table("demo.null_test_iii.summary").filter("cons_acct_key = '9999'").collect()
...
def run():
    ...
    config = {
        "source_table": "demo.null_test_iii.accounts",
        "destination_table": "demo.null_test_iii.summary",
        "latest_history_table": "demo.null_test_iii.latest_summary",
